...
import { resolveSkin, resolveTheme, resolveThemeSeed, typeScale } from '../src/prefs/resolve';
import { contrastRatio, WCAG_AA_TEXT } from '../src/theme';
import { groupReceipts, summarizeReceipts, UNDO_LABEL } from '../src/pipeline/receipts';
...
describe('skin resolution', () => {
  ...
  test('a feed overrides field by field, not wholesale', () => {
    ...
    assert.equal(skin.media, prefs.skin.media, 'untouched fields keep the global value');
    assert.equal(skin.showWhyChips, prefs.skin.showWhyChips);
  });

  test('an explicit false overrides a true default', () => {
    const prefs = defaultPreferences();
    assert.equal(resolveSkin(prefs, { showWhyChips: false }).showWhyChips, false);
  });
...
describe('theme resolution', () => {
  ...
  test('ignores the OS when the reader pinned a theme', () => {
    const prefs = { ...defaultPreferences(), followSystemTheme: false, darkThemeId: 'terminal' };
    assert.equal(resolveThemeSeed(prefs, 'light').id, 'terminal');
  });

  test('a feed can pin its own theme', () => {
    const prefs = defaultPreferences();
    assert.equal(resolveThemeSeed(prefs, 'dark', { themeId: 'sepia' }).id, 'sepia');
  });
  ...
  test('resolved themes are always readable', () => {
    ...
    for (const scheme of ['light', 'dark'] as const) {
      ...
      assert.ok(contrastRatio(theme.text.primary, theme.background.default) >= WCAG_AA_TEXT);
    }
  });
});
...
describe('type scale', () => {
  ...
  test('sizes land on half points, so text never renders blurry', () => {
    ...
    for (const v of [scale.caption, scale.meta, scale.body, scale.title, scale.display]) {
...
describe('receipt grouping', () => {
  const receipts: DropReceipt[] = [
    { itemId: '1', authorFid: 1, cause: 'muted-keyword', detail: 'Matched "election".', rule: 'election' },
    { itemId: '2', authorFid: 2, cause: 'muted-keyword', detail: 'Matched "election".', rule: 'election' },
    { itemId: '3', authorFid: 3, cause: 'muted-group', detail: 'Discovery is off.', rule: 'discovery' },
    { itemId: '4', authorFid: 4, cause: 'hidden-reply', detail: 'Replies are hidden.' },
  ];
  ...
  test('same rule under different causes stays separate', () => {
    const groups = groupReceipts([
      { itemId: 'a', authorFid: 1, cause: 'muted-group', detail: 'x', rule: 'promoted' },
      { itemId: 'b', authorFid: 1, cause: 'muted-reason', detail: 'y', rule: 'promoted' },
    ]);
    assert.equal(groups.length, 2);
    assert.equal(new Set(groups.map((g) => g.key)).size, 2, 'keys are unique for React');
  });
  ...
  test('the summary line is absent when nothing was hidden', () => {
    ...
    assert.equal(summarizeReceipts(receipts.slice(0, 1)), '1 cast hidden on this page.');
