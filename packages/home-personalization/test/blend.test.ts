...
describe('interleaveByWeight', () => {
  test('honours the proportion at the top of the page, not only in the limit', () => {
    ...
    assert.equal(
      share(firstTen, 'a'),
      0.7,
      'the first window is already 70/30',
    );
  ...
  test('weights are relative, so any scale expresses the same blend', () => {
    const asPercent = interleaveByWeight(
      [
        { weight: 75, items: make('a', 40) },
        { weight: 25, items: make('b', 40) },
      ],
      8,
    );
    const asSliders = interleaveByWeight(
      [
        { weight: 3, items: make('a', 40) },
        { weight: 1, items: make('b', 40) },
      ],
      8,
    );
    assert.deepEqual(
      asPercent.map((i) => i.id),
      asSliders.map((i) => i.id),
    );
  });
  ...
  test('emits every item exactly once', () => {
    const out = interleaveByWeight(
      [
        { weight: 2, items: make('a', 13) },
        { weight: 1, items: make('b', 7) },
      ],
      6,
    );
  ...
  test('an exhausted source hands its slots back rather than leaving gaps', () => {
    const out = interleaveByWeight(
      [
        { weight: 1, items: make('a', 2) },
        { weight: 1, items: make('b', 10) },
      ],
      4,
    );
    assert.equal(out.length, 12);
    assert.deepEqual(
      out.slice(-4).map((i) => i.id.startsWith('b')),
      [true, true, true, true],
    );
  });
  ...
  test('preserves each source order', () => {
    const out = interleaveByWeight(
      [
        { weight: 1, items: make('a', 5) },
        { weight: 1, items: make('b', 5) },
      ],
      4,
    );
  ...
  test('a single live part is a pass-through', () => {
    ...
    assert.deepEqual(
      interleaveByWeight([{ weight: 3, items }]).map((i) => i.id),
      items.map((i) => i.id),
    );
  });
  ...
  test('is deterministic', () => {
    const build = () =>
      interleaveByWeight(
        [
          { weight: 5, items: make('a', 20) },
          { weight: 3, items: make('b', 20) },
          { weight: 2, items: make('c', 20) },
        ],
        10,
      );
    assert.deepEqual(
